...
class SmartWrapIndentingKtTest {
    @Test
    fun `given an empty String, when cutoff size is more than or equal to 3, then return empty string`() {
        "".smartWrap(3).shouldBeEmpty()
        "".smartWrap(5).shouldBeEmpty()
    }
    ...
    fun `when string with no spaces is longer than twice maxCharPerLine, then return string split in multiple lines with hyphens`() {
        ...
        myString.smartWrap(6).shouldBeEqual(
            "12345-\n" +
                "   67-${lineSeparator()}" +
                "   89-${lineSeparator()}" +
                "   01-${lineSeparator()}" +
    ...
    fun `when string with spaces is longer than maxCharPerLine, then return string split in 2 lines on spaces`() {
        val myString = "12345 789"
        myString.smartWrap(6).shouldBeEqual("12345${lineSeparator()}   789")
    }
    ...
    fun `given last line in multiline paragraph is completely full, when appending string, then append string to new line without preceding space`() {
        val myString = "123456 789"
        myString.smartWrap(6).shouldBeEqual("123456${lineSeparator()}   789")
    }
    ...
    companion object {
        private fun String.smartWrap(maxCharsPerLine: Int): String = SmartWrapIndenting(maxCharsPerLine).invoke(this)
    }
}

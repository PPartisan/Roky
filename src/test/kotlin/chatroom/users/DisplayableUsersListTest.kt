...
class DisplayableUsersListTest {
    ...
    @Test
    fun `when username exist for profile id, then emit username`()=runTest {
    ...
    @Test
    fun `when username does not exist, then user is unknown`()=runTest {
        every { readPresence.observe() } coAnswersDelayed {
            flowOf( PresenceResult.ok(setOf("id")))
        }
        every { readProfiles.observe() } coAnswersDelayed {
    ...
    @Test
    fun `when username for profile id is blank, then user is unknown`()= runTest {
        every { readPresence.observe() } coAnswersDelayed {
            flowOf( PresenceResult.ok(setOf("id")))
        }
        every { readProfiles.observe() } coAnswersDelayed {
    ...
    @Test
    fun `when multiple users are in the chatroom, then show usernames for users in chatroom`()= runTest {
        every { readPresence.observe() } coAnswersDelayed {
